...
(async () => {
  ...
  ok("[زائر] ختم الإصدار 12.9", V.txt().includes("الإصدار 12.9"));
  ok("[زائر] مقصد نظرة عامة بالشريط", V.txt().includes("نظرة عامة"));
  ok("[زائر] زر الأدوات بالرأس", V.txt().includes("⋯ أدوات"));
  ok("[زائر] أزرار الأدوات لم تعد ظاهرة بالرأس", !V.txt().includes("ربط GitHub"));

  await V.click("نظرة عامة");

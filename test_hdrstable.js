...
(async()=>{
  ...
  ok("[جوال] الشعار في سطر مستقل أعلى العنوان", html.includes("header .hdr-id { flex-direction: column;"));
  ok("[جوال] الفجوة مخفية بالجوال (لا حاجة لها)", html.includes("header .hdr-title .hdr-gap { display: none; }"));
  ok("[جوال] العنوان بعرض الشاشة كاملاً", html.includes('header .hdr-id > div:not(.hdr-logo) { width: 100%;'));
  ok("[جوال] العنوان يملأ السطر بلا توزيع متوازن", !html.includes("text-wrap: balance"));
  ok("[جوال] الشعار محاذٍ لأقصى اليمين", html.includes("align-items: flex-start !important"));
  ok("[جوال] حافة شريط الأزرار متلاشية لا مقطوعة", html.includes("mask-image: linear-gradient(to left"));
  ok("[مكتب] الفجوة تطفو يساراً فيلتفّ النص حولها", html.includes("header .hdr-title .hdr-gap { float: left;"));
  ...
  ok("[مكتب] صف الهوية = الشعار والعنوان فقط", D.querySelectorAll("header .hdr-id > *").length===2);
  ok("[مكتب] لا صف أزرار طافٍ فوق الصورة", !D.querySelector("header .hdr-actions"));

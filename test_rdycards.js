...
const click=async(t,ms)=>{const b=Array.from(D.querySelectorAll("button")).find(x=>(x.getAttribute("title")||"")===t)||Array.from(D.querySelectorAll("button")).find(x=>(x.textContent||"").includes(t));if(b){b.click();await wait(ms||1200);}return !!b;};
...
(async()=>{
  ...
  ok("بطاقة «مراكز يوجد بها نقص جزئي بالجاهزية»", t.includes("مراكز يوجد بها نقص جزئي بالجاهزية"));
  ok("بطاقة «مراكز يوجد بها عجز مؤثر»", t.includes("مراكز يوجد بها عجز مؤثر"));
  ok("لا أثر للتسميات القديمة", !t.includes("جاهزية مكتملة (أخضر)") && !t.includes("نقص متطلبات المركز (أصفر)") && !t.includes("عجز كامل") && !t.includes("نقص بسيط") && !t.includes("شعب مغطاة بقص الخواتم") && !t.includes("شعب مغطاة بالمفتاح العادي"));
  ok("نسبة مئوية تحت كل بطاقة (ثلاث نسب)", (t.match(/% من المراكز/g)||[]).length >= 3);
  ok("ألوان البطاقات أغمق", html.includes("#DFF3E8") && html.includes("#FCF0D9") && html.includes("#FBE3E5"));
  ok("بطاقة قص الخواتم بالتسمية الجديدة ونسبتها", t.includes("شعب يوجد بها جهاز قص الخواتم") && /% من الشعب/.test(t));
  ok("بطاقة مفتاح المصاعد العادي مستقلة", t.includes("شعب مغطاة بمفتاح المصاعد العادي"));
  ok("بطاقة مفتاح المصاعد الإلكتروني مستقلة", t.includes("شعب مغطاة بمفتاح المصاعد الإلكتروني"));
  ok("لكل بطاقة نسبتها (خمس نسب من الشعب/المراكز)", (t.match(/% من الشعب/g)||[]).length === 3);
